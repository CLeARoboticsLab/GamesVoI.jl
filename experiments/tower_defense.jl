...
""" Nomenclature
    ...
    x[Block(5)]                : [x(s¹), y(s¹), z(s¹)]|₀ᵀ, P1's action given signal s¹=0
...
function solve_full_tower_defense_game(θ, pₖ)  
    ...
    fs = [# Stage 2
          ...
          (x,θ) ->  x[Block(1)]'*diagm(vcat(θ[1], 1-θ[1], θ[2]))*x[Block(2)], 
          (x,θ) ->  x[Block(1)]'*diagm(vcat(θ[3], 1-θ[3], θ[4]))*x[Block(3)],
          (x,θ) ->  x[Block(1)]'*diagm(vcat(θ[5], 1-θ[5], θ[6]))*x[Block(4)],
          ...
          (x,θ) ->  x[Block(5)]'*diagm(vcat(θ[1], 1-θ[1], θ[2]))*x[Block(6)], 
          (x,θ) ->  x[Block(5)]'*diagm(vcat(θ[3], 1-θ[3], θ[4]))*x[Block(7)],
          (x,θ) ->  x[Block(5)]'*diagm(vcat(θ[5], 1-θ[5], θ[6]))*x[Block(8)],
    ...
    function g9(x, θ) # placeholder constraint to keep # same
        [sum(x[Block(1)]) - 1]
    end
